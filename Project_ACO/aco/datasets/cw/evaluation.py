...
from .scorer import cw_llm_score
from aco.genome import ROLE_DESCRIPTIONS, CAPABILITY_DESCRIPTIONS
...
def evaluate_mas(
    ...
    _judge_model = judge_model or _DEFAULT_JUDGE_MODEL

    # Judge always needs NVIDIA API key (scorer.py uses OpenAI client directly).
    api_key = os.environ.get("NVIDIA_API_KEY")
    if not api_key:
        raise RuntimeError(
            "NVIDIA_API_KEY is required for the CW LLM judge, even when LOCAL_LLM=1."
        )
    base_url = os.environ.get("NVIDIA_API_BASE", "https://integrate.api.nvidia.com/v1")

    if use_local:
        from aco.local_llm import patch_autogen_for_local_llm, register_on_agent, local_llm_config
    ...
    if task_name != "CreativeWriting" or not evaluation_batch:
        ...
        for sample_idx, sample in enumerate(evaluation_batch, 1):
            ...
            try:
                ...
                # Scale 1–10 → 10–100 to match ACO fitness range of TP/NP
        ...
        if task_name == "CreativeWriting" and evaluation_batch:
            cw_payload: Dict[str, Any] = {
                ...
                "note": (
                    ...
                    "(the inner ACO signal before the topology bonus)."
                ),
            }
        else:
            cw_payload = None
