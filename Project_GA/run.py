#!/usr/bin/env python3
"""
Run TravelPlanner GA, Natural Plan GA, and Creative Writing GA via the unified ``ga``
...
mirrors each track into ``ga_final/run_latest.json``, ``ga_np/run_latest.json``,
and ``ga_cw/run_latest.json``.
"""
...
def _save_best_config(log_dir: Path, track: str, best, log: list, params: dict) -> None:
    """Print and save the best GA config to a checkpoint file before final evaluation."""
    best_fitness = log[-1]["best"] if log else None

    print(f"\n{'#'*60}")
    print(f"  BEST {track.upper()} CONFIG (post-GA, pre-eval)")
    print(f"  Config    : {best}")
    if best:
...
def main() -> None:
    p = argparse.ArgumentParser(
        description=(
            "Run TravelPlanner (TP), Natural Plan (NP), and/or Creative Writing (CW) GA "
    ...
    run_mode.add_argument(
        "--tp-only",
        action="store_true",
        help="Run only the TravelPlanner (ga_final) leg.",
    )
    run_mode.add_argument(
        "--np-only",
        action="store_true",
        help="Run only the Natural Plan (ga_np) leg.",
    )
    run_mode.add_argument(
        "--cw-only",
        action="store_true",
        help="Run only the Creative Writing (ga_cw) leg.",
    )
    ...
    p.add_argument("--pop-size", type=int, default=20, help="GA population size.")
    p.add_argument("--generations", type=int, default=10, help="Number of GA generations.")
    p.add_argument("--cxpb", type=float, default=0.5, help="Crossover probability.")
    p.add_argument("--mutpb", type=float, default=0.2, help="Mutation probability per attribute.")
    p.add_argument("--num-agents", type=int, default=4)
    p.add_argument("--tp-batch", type=int, default=20, help="TravelPlanner mini-batch size (used for GA optimization when --tp-eval-batch is not set)")
    p.add_argument("--tp-split", choices=("train", "validation", "test"), default="validation")
    p.add_argument("--tp-batch-seed", type=int, default=42)
    p.add_argument("--tp-eval-batch", type=int, default=None, help="If set, evaluate the best GA config on this many separate queries (distinct from the train batch)")
    ...
    p.add_argument("--cw-train", action="store_true", help="Use the 5 official ToT training tasks for GA optimization instead of sampling from the 95 eval tasks.")
    ...
    p.add_argument("--cw-batch", type=int, default=20, help="CW samples per evaluation (0 = all 95 eval tasks). Used for GA optimization when --cw-train is NOT set.")
    ...
    p.add_argument(
        ...
        action="store_true",
        help="Skip writing ga_final/run_latest.json, ga_np/run_latest.json, and ga_cw/run_latest.json",
    )
    args = p.parse_args()
    ...
    if not args.skip_tp:
        from ga.main_ga import run_ga
        from ga.datasets.tp.loader import load_batch as tp_load_batch
        from ga.datasets.tp.evaluation import evaluate_mas as tp_evaluate_mas

        tp_params = {**ga_base_params, "task_name": "TravelPlanner"}
        best_tp, log_tp, batch_tp = run_ga(
            **tp_params,
        ...
        out["travel_planner"] = {
            "ga_parameters": {**tp_params, "batch_size": args.tp_batch, "tp_split": args.tp_split, "batch_seed": args.tp_batch_seed},
            "eval_parameters": {
            ...
            "best_fitness_last_gen": log_tp[-1]["best"] if log_tp else None,
    ...
    if not args.skip_np:
        from ga.main_ga import run_ga
        from ga.datasets.np.loader import load_np_batch, load_np_batch_mixed
        from ga.datasets.np.evaluation import evaluate_mas as np_evaluate_mas

        np_params = {**ga_base_params, "task_name": "NaturalPlan"}
        ...
        best_np, log_np, batch_np = run_ga(
            **np_params,
        ...
        if args.np_mixed:
            np_ga_params["np_per_kind"] = args.np_per_kind
            np_ga_params["batch_size"] = args.np_per_kind * 3
            np_ga_params["eval_per_kind"] = args.np_eval_per_kind
            np_ga_params["eval_seed"] = args.np_eval_seed
        else:
            np_ga_params["batch_size"] = args.np_batch

        out["natural_plan"] = {
            "ga_parameters": np_ga_params,
            "best_fitness_last_gen": log_np[-1]["best"] if log_np else None,
    ...
    if not args.skip_cw:
        from ga.main_ga import run_ga
        from ga.datasets.cw.loader import load_cw_batch, load_cw_train_batch
        from ga.datasets.cw.evaluation import evaluate_mas as cw_evaluate_mas

        cw_params = {**ga_base_params, "task_name": "CreativeWriting"}
        ...
        best_cw, log_cw, batch_cw = run_ga(
            **cw_params,
        ...
        if args.cw_train:
            cw_ga_params["train_split"] = True
            cw_ga_params["train_batch_size"] = args.cw_train_batch if args.cw_train_batch else 5
            cw_ga_params["train_seed"] = args.cw_train_seed
            cw_ga_params["eval_batch_size"] = args.cw_eval_batch
            cw_ga_params["eval_seed"] = args.cw_eval_seed
        else:
            cw_ga_params["batch_size"] = args.cw_batch
            cw_ga_params["batch_seed"] = args.cw_batch_seed

        out["creative_writing"] = {
            "ga_parameters": cw_ga_params,
            "best_fitness_last_gen": log_cw[-1]["best"] if log_cw else None,
    ...
    log_path = args.log_dir / f"{stem}_{kind_tag}_ga_run.json"
    ...
    if not args.no_mirror_folder_logs:
        ...
        if out.get("travel_planner") is not None:
            ...
            (root / "ga_final").mkdir(parents=True, exist_ok=True)
            (root / "ga_final" / "run_latest.json").write_text(
                json.dumps(_json_safe(tp_mirror), indent=2), encoding="utf-8"
            )
            print(f"Wrote {root / 'ga_final' / 'run_latest.json'}")
        if out.get("natural_plan") is not None:
            ...
            (root / "ga_np").mkdir(parents=True, exist_ok=True)
            (root / "ga_np" / "run_latest.json").write_text(
                json.dumps(_json_safe(np_mirror), indent=2), encoding="utf-8"
            )
            print(f"Wrote {root / 'ga_np' / 'run_latest.json'}")
        if out.get("creative_writing") is not None:
            ...
            (root / "ga_cw").mkdir(parents=True, exist_ok=True)
            (root / "ga_cw" / "run_latest.json").write_text(
                json.dumps(_json_safe(cw_mirror), indent=2), encoding="utf-8"
            )
            print(f"Wrote {root / 'ga_cw' / 'run_latest.json'}")
