...
class GenomeEncoder:
    ...
    def encode(self, individual: MASConfiguration) -> np.ndarray:
        ...
        for a in individual.agents:
            ...
            # safer to map using index lookup on the lists from genome module
            try:
...
class SMBOOptimizer:
    ...
    def run(self, evaluate_fn: Callable[[MASConfiguration], float],
        ...
        evaluate_fn: function that accepts a MASConfiguration and returns a scalar score.
        ...
        best_feat = self.X[best_idx] if best_idx is not None else None
        ...
        result = {
            ...
            'evaluations': len(self.y),
            'all_scores': self.y,
            # Detailed history: list of {config, score} in evaluation order
            'history': [
...
if __name__ == '__main__':
    print('This module provides SMBOOptimizer; run via run_smbo.py')
