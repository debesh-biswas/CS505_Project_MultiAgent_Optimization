#!/usr/bin/env python3
"""
Run TravelPlanner SMBO, Natural Plan SMBO, and Creative Writing SMBO via the unified ``smbo``
...
mirrors each track into ``smbo_final/run_latest.json``, ``smbo_np/run_latest.json``,
and ``smbo_cw/run_latest.json``.
"""
...
def _save_best_config(log_dir: Path, track: str, best, log: list, params: dict) -> None:
    """Print and save the best SMBO config to a checkpoint file before final evaluation."""
    best_fitness = log[-1]["best"] if log else None

    print(f"\n{'#'*60}")
    print(f"  BEST {track.upper()} CONFIG (post-SMBO, pre-eval)")
    print(f"  Config    : {best}")
    if best:
...
def main() -> None:
    p = argparse.ArgumentParser(
        description=(
            "Run TravelPlanner (TP), Natural Plan (NP), and/or Creative Writing (CW) SMBO "
    ...
    run_mode.add_argument(
        "--tp-only",
        action="store_true",
        help="Run only the TravelPlanner (smbo_final) leg.",
    )
    run_mode.add_argument(
        "--np-only",
        action="store_true",
        help="Run only the Natural Plan (smbo_np) leg.",
    )
    run_mode.add_argument(
        "--cw-only",
        action="store_true",
        help="Run only the Creative Writing (smbo_cw) leg.",
    )
    p.add_argument("--skip-tp", action="store_true", help="Skip TravelPlanner.")
    p.add_argument("--skip-np", action="store_true", help="Skip Natural Plan.")
    p.add_argument("--skip-cw", action="store_true", help="Skip Creative Writing.")
    p.add_argument("--n-init", type=int, default=30, help="Number of random initial evaluations.")
    p.add_argument("--iterations", type=int, default=20, help="Number of surrogate-guided iterations.")
    p.add_argument("--pool-size", type=int, default=500, help="Candidate pool size for acquisition.")
    p.add_argument("--top-k", type=int, default=5, help="Top-k candidates to evaluate per iteration.")
    p.add_argument("--kappa", type=float, default=1.0, help="UCB exploration coefficient.")
    p.add_argument("--num-agents", type=int, default=4)
    p.add_argument("--tp-batch", type=int, default=20, help="TravelPlanner mini-batch size (used for SMBO optimization when --tp-eval-batch is not set)")
    p.add_argument("--tp-split", choices=("train", "validation", "test"), default="validation")
    p.add_argument("--tp-batch-seed", type=int, default=42)
    p.add_argument("--tp-eval-batch", type=int, default=None, help="If set, evaluate the best SMBO config on this many separate queries (distinct from the train batch)")
    ...
    p.add_argument("--cw-train", action="store_true", help="Use the 5 official ToT training tasks for SMBO optimization instead of sampling from the 95 eval tasks.")
    ...
    p.add_argument("--cw-batch", type=int, default=20, help="CW samples per evaluation (0 = all 95 eval tasks). Used for SMBO optimization when --cw-train is NOT set.")
    ...
    p.add_argument(
        ...
        action="store_true",
        help="Skip writing smbo_final/run_latest.json, smbo_np/run_latest.json, and smbo_cw/run_latest.json",
    )
    args = p.parse_args()
    ...
    if not args.skip_tp:
        from smbo.main_smbo import run_smbo
        from smbo.datasets.tp.loader import load_batch as tp_load_batch
        from smbo.datasets.tp.evaluation import evaluate_mas as tp_evaluate_mas

        tp_params = {**smbo_base_params, "task_name": "TravelPlanner"}
        best_tp, log_tp, batch_tp = run_smbo(
            **tp_params,
        ...
        out["travel_planner"] = {
            "smbo_parameters": {**tp_params, "batch_size": args.tp_batch, "tp_split": args.tp_split, "batch_seed": args.tp_batch_seed},
            "eval_parameters": {
    ...
    if not args.skip_np:
        from smbo.main_smbo import run_smbo
        from smbo.datasets.np.loader import load_np_batch, load_np_batch_mixed
        from smbo.datasets.np.evaluation import evaluate_mas as np_evaluate_mas

        np_params = {**smbo_base_params, "task_name": "NaturalPlan"}
        ...
        best_np, log_np, batch_np = run_smbo(
            **np_params,
        ...
        if args.np_mixed:
            np_smbo_params["np_per_kind"] = args.np_per_kind
            np_smbo_params["batch_size"] = args.np_per_kind * 3
            np_smbo_params["eval_per_kind"] = args.np_eval_per_kind
            np_smbo_params["eval_seed"] = args.np_eval_seed
        else:
            np_smbo_params["batch_size"] = args.np_batch

        out["natural_plan"] = {
            "smbo_parameters": np_smbo_params,
    ...
    if not args.skip_cw:
        from smbo.main_smbo import run_smbo
        from smbo.datasets.cw.loader import load_cw_batch, load_cw_train_batch
        from smbo.datasets.cw.evaluation import evaluate_mas as cw_evaluate_mas

        cw_params = {**smbo_base_params, "task_name": "CreativeWriting"}
        ...
        best_cw, log_cw, batch_cw = run_smbo(
            **cw_params,
        ...
        if args.cw_train:
            cw_smbo_params["train_split"] = True
            cw_smbo_params["train_batch_size"] = args.cw_train_batch if args.cw_train_batch else 5
            cw_smbo_params["train_seed"] = args.cw_train_seed
            cw_smbo_params["eval_batch_size"] = args.cw_eval_batch
            cw_smbo_params["eval_seed"] = args.cw_eval_seed
        else:
            cw_smbo_params["batch_size"] = args.cw_batch
            cw_smbo_params["batch_seed"] = args.cw_batch_seed

        out["creative_writing"] = {
            "smbo_parameters": cw_smbo_params,
    ...
    log_path = args.log_dir / f"{stem}_{kind_tag}_smbo_run.json"
    ...
    if not args.no_mirror_folder_logs:
        ...
        if out.get("travel_planner") is not None:
            ...
            (root / "smbo_final").mkdir(parents=True, exist_ok=True)
            (root / "smbo_final" / "run_latest.json").write_text(
                json.dumps(_json_safe(tp_mirror), indent=2), encoding="utf-8"
            )
            print(f"Wrote {root / 'smbo_final' / 'run_latest.json'}")
        if out.get("natural_plan") is not None:
            ...
            (root / "smbo_np").mkdir(parents=True, exist_ok=True)
            (root / "smbo_np" / "run_latest.json").write_text(
                json.dumps(_json_safe(np_mirror), indent=2), encoding="utf-8"
            )
            print(f"Wrote {root / 'smbo_np' / 'run_latest.json'}")
        if out.get("creative_writing") is not None:
            ...
            (root / "smbo_cw").mkdir(parents=True, exist_ok=True)
            (root / "smbo_cw" / "run_latest.json").write_text(
                json.dumps(_json_safe(cw_mirror), indent=2), encoding="utf-8"
            )
            print(f"Wrote {root / 'smbo_cw' / 'run_latest.json'}")
